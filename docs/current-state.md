...
| Android release | Internal testing | Google Play app exists, AAB version code 9 is active on the internal testing track, and tester install from Play is confirmed. |
...
- Mobile Search can show `TVLore Picks`, personalized `Recommended picks`, country-aware `Available to stream`, and country-aware `Popular in your country` entry cards, while detailed streaming provider icons stay in detail screens.
...
- Mobile Library grouped episode sections open show detail from the show title, season detail from the season heading, episode detail from episode rows, and keep a separate expand/collapse control.
...
-> Available to stream route for availability-first country-aware discovery
-> GET /discovery/popular
